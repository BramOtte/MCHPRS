...
impl ThreadedBackend {
    pub fn tick_node(&mut self, priority: TickPriority, group_id: u32, node_id: NodeId) {
        let node = &mut self.nodes[node_id];
        node.pending_tick = 255;
        ...
        match node.ty {
            NodeType::Repeater { delay, .. } => {
                ...
                if node.powered && !should_be_powered {
                    self.set_node(priority, node_id, false, 0);
                    ...
                    self.set_node(priority, node_id, true, 15);
                }
            }
            NodeType::Torch => {
                let should_be_powered = !get_bool_input(node);
                if node.powered != should_be_powered {
                    self.set_node(priority, node_id, should_be_powered, bool_to_ss(should_be_powered));
                }
            }
            NodeType::Comparator {
                mode, far_input, ..
                ...
                if new_strength != old_strength {
                    self.set_node(priority, node_id, new_strength > 0, new_strength);
                }
            }
            NodeType::Lamp => {
