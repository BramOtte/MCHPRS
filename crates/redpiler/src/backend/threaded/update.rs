...
pub(super) fn update_node(
    ...
    match node.ty {
        NodeType::Repeater {
            ...
            if node.locked || node.pending_tick < 255 {
                return;
            }
            ...
            if should_be_powered != node.powered {
                ...
                schedule_tick(scheduler, node.group_id, node_id, node, delay as usize, priority);
            }
        }
        NodeType::Torch => {
            if node.pending_tick < 255 {
                return;
            }
            let should_be_powered = !get_bool_input(node);
            if node.powered != should_be_powered {
                schedule_tick(scheduler, node.group_id, node_id, node, 1, TickPriority::Normal);
            }
        }
        NodeType::Comparator {
            mode,
            ...
            if output_power != old_strength {
                ...
                schedule_tick(scheduler, node.group_id, node_id, node, 1, priority);
            }
        }
        NodeType::Lamp |
        // => {
        ...
        //         schedule_tick(scheduler, node.group_id, node_id, node, 2, TickPriority::Normal);
